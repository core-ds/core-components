...
При особой необходимости, для рендеринга по условию в шаблонах используйте компонент [Mq](?path=/docs/components-mq--docs), который основан на `window.matchMedia API`:
