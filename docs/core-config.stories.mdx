...
Для того чтобы не переопределять компоненты по отдельности, можно воспользоваться контекстом - компоненты автоматически получат значение из него (доступен в core-components с 48 версии).
...
В данный момент доступные следующие параметры для настройки:
