...
Пакет `Skeleton` предоставляет хук `useSkeleton`, с помощью которого можно "скелетонизировать" текст, а также кастомизировать его отображение.<br/>
*<b>Обратите особое внимание на то, что у текста должны быть заданы `fontSize` и `lineHeight`</b>*.
