import { ArgsTabs } from 'storybook/blocks';
import { PureCell as PureCellTS } from '../component';

## Подключение

```jsx
// Атомарные
import {
    PureCellComponent,
    Main,
    Graphics,
    Content,
    Text,
    Amount,
    AmountTitle,
    Addon,
    Footer,
    ExtraSubtitle,
    FooterButton,
    Comment,
    Category,
} from '@alfalab/core-components/pure-cell';

// Компаунд
import { PureCell } from '@alfalab/core-components/pure-cell';
```

## Типы

Типы свойств компонента и его атомарных частей доступны из корневого экспорта:

```tsx
import type {
    PureCellProps,
    PureCellMainProps,
    PureCellGraphicsProps,
    PureCellContentProps,
    PureCellTextProps,
    PureCellAmountProps,
    PureCellAmountTitleProps,
    PureCellAddonProps,
    PureCellFooterProps,
    PureCellExtraSubtitleProps,
    PureCellFooterButtonProps,
    PureCellCommentProps,
    PureCellCategoryProps,
} from '@alfalab/core-components/pure-cell';
```

При использовании отдельного пакета типы доступны из его корневого экспорта:

```tsx
import type { PureCellTextProps } from '@alfalab/core-components-pure-cell';
```

## Использование dataTestId

В компоненте используются модификаторы для `dataTestId`.
Для удобного поиска элементов можно воспользоваться функцией `getPureCellTestIds`.
Импорт из `@alfalab/core-components/pure-cell/shared`.

Функция возвращает объект:

```jsx
{
    pureCell: dataTestId,
    addon: `${dataTestId}-addon`,
    amount: `${dataTestId}-amount`,
    amountText: `${dataTestId}-amount-text`,
    amountTitle: `${dataTestId}-amount-title`,
    coreAmountTitle: `${dataTestId}-core-amount-title`,
    categoryName: `${dataTestId}-category-name`,
    categoryPercent: `${dataTestId}-category-percent`,
    categoryRightAddon: `${dataTestId}-category-right-addon`,
    content: `${dataTestId}-content`,
    footer: `${dataTestId}-footer`,
    footerButton: `${dataTestId}-button`,
    extraSubtitle: `${dataTestId}-footer-title`,
    graphics: `${dataTestId}-graphics`,
    main: `${dataTestId}-main`,
    textContent: `${dataTestId}-text_content`,
    textValue: `${dataTestId}-text_value`,
};
```

## Свойства

<ArgsTabs
    components={{
        PureCell: PureCellTS,
        Addon: PureCellTS.Addon,
        Amount: PureCellTS.Amount,
        AmountTitle: PureCellTS.AmountTitle,
        Category: PureCellTS.Category,
        Comment: PureCellTS.Comment,
        Content: PureCellTS.Content,
        Footer: PureCellTS.Footer,
        FooterButton: PureCellTS.FooterButton,
        ExtraSubtitle: PureCellTS.ExtraSubtitle,
        Graphics: PureCellTS.Graphics,
        Main: PureCellTS.Main,
        Text: PureCellTS.Text,
    }}
/>
