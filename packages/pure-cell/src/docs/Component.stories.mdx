import { Meta, Story, Markdown } from '@storybook/addon-docs';
import { text, boolean, select } from '@storybook/addon-knobs';
import { ComponentHeader, Tabs } from 'storybook/blocks';

import { Circle } from '@alfalab/core-components-icon-view/circle';
import { PureCell } from '@alfalab/core-components-pure-cell';
import { Typography } from '@alfalab/core-components-typography';
import { StatusBadge } from '@alfalab/core-components-status-badge';
import { StarMIcon } from '@alfalab/icons-glyph/StarMIcon';
import { ChevronForwardMIcon } from '@alfalab/icons-glyph/ChevronForwardMIcon';

import Description from './description.mdx';
import Development from './development.mdx';
import Changelog from '../../CHANGELOG.md?raw';
import {
    stylesStringToObj,
    getQueryParam,
} from '@alfalab/core-components-screenshot-utils/screenshots-story/utils';

export const VIEWS = ['primary', 'secondary', 'tertiary', 'link', 'ghost'];

<Meta title='Components/PureCell' component={PureCell} id='PureCell' />

{/* Canvas */}

<Story name='PureCell'>
    {React.createElement(() => {
        const verticalPadding = select(
            'verticalPadding',
            ['airy', 'default', 'compact', 'tiny', 'none'],
            'default',
        );
        const verticalTopPadding = select(
            'verticalPadding.top',
            ['airy', 'default', 'compact', 'tiny', 'none', undefined],
            undefined,
        );
        const verticalBottomPadding = select(
            'verticalPadding.bottom',
            ['airy', 'default', 'compact', 'tiny', 'none', undefined],
            undefined,
        );
        const verticalPaddingPrepared = (verticalTopPadding || verticalBottomPadding) ? {top: verticalTopPadding, bottom: verticalBottomPadding } : verticalPadding

        const horizontalPaddingSide = select(
            'horizontalPadding',
            ['left', 'right', 'both', 'none'],
            'none',
        );
        const horizontalLeftPadding = select('horizontalPadding.left', [0, 16, 20, undefined], undefined);
        const horizontalRightPadding = select('horizontalPadding.right', [0, 16, 20, undefined], undefined);
        const horizontalPadding = (horizontalLeftPadding !== undefined || horizontalRightPadding !== undefined) ? { left: horizontalLeftPadding, right: horizontalRightPadding } : horizontalPaddingSide;
        const graphicPadding = select(
            'graphicPadding',
            ['max', 'airy', 'default', 'compact', 'tiny', 'none', undefined],
            undefined,
        );
        const direction = select('direction', ['vertical', 'horizontal'], 'horizontal');
        const weight = select('weight', ['bold', 'normal'], 'normal');
        const view = select('view', ['component', 'primary-small'], 'component');
        const viewAmount = select(
            'viewAmount',
            ['default', 'withZeroMinorPart'],
            'withZeroMinorPart',
        );
        const color = select(
            'color',
            [
                'tertiary',
                'disabled',
                'accent',
                'primary',
                'attention',
                'positive',
                'secondary',
                'tertiary-inverted',
                'primary-inverted',
                'secondary-inverted',
                'link',
                'negative',
            ],
            'primary',
        );
        const value = text('value', '100000000');
        return (
            <div
                style={{
                    display: 'flex',
                    justifyContent: 'space-around',
                }}
            >
                <div
                    style={{
                        display: 'flex',
                        gap: '20px',
                        flexWrap: 'nowrap',
                        flexDirection: 'column',
                        flexBasis: '250px',
                    }}
                >
                    <PureCell
                        href={text('href', '')}
                        verticalPadding={verticalPaddingPrepared}
                        horizontalPadding={horizontalPadding}
                        direction={direction}
                    >
                        <PureCell.Graphics graphicPadding={graphicPadding}>
                            <StarMIcon />
                        </PureCell.Graphics>
                        <PureCell.Content>
                            <PureCell.Main>
                                <PureCell.Text
                                    titleColor='primary'
                                    view='component-primary'
                                    value='Value'
                                    valueColor='secondary'
                                    rowLimit={1}
                                >
                                    Long title for example
                                </PureCell.Text>
                                <PureCell.Text
                                    titleColor='secondary'
                                    view='primary-small'
                                    value='SubValue'
                                    valueColor='secondary'
                                    rowLimit={1}
                                >
                                    Long label for example
                                </PureCell.Text>
                            </PureCell.Main>
                        </PureCell.Content>
                    </PureCell>
                    <PureCell
                        href={text('href', '')}
                        verticalPadding={verticalPaddingPrepared}
                        horizontalPadding={horizontalPadding}
                        direction={direction}
                    >
                        <PureCell.Content>
                            <PureCell.Main>
                                <PureCell.Text titleColor='secondary' view='primary-small'>
                                    Product Name ··0000
                                </PureCell.Text>
                                <PureCell.Amount
                                    view={viewAmount}
                                    value={value}
                                    minorUnits='100'
                                    currency='RUR'
                                    weight={weight}
                                    color={color}
                                />
                            </PureCell.Main>
                            <PureCell.Footer>
                                <PureCell.Text titleColor='secondary' view='primary-small'>
                                    Caption Secondary
                                </PureCell.Text>
                                <PureCell.FooterButton>Action</PureCell.FooterButton>
                            </PureCell.Footer>
                        </PureCell.Content>
                    </PureCell>
                    <PureCell
                        href={text('href', '')}
                        verticalPadding={verticalPaddingPrepared}
                        horizontalPadding={horizontalPadding}
                        direction={direction}
                    >
                        <PureCell.Content>
                            <PureCell.Main>
                                <PureCell.Text titleColor='secondary' view='primary-small'>
                                    Общий баланс
                                </PureCell.Text>
                                <PureCell.AmountTitle
                                    view={viewAmount}
                                    value={value}
                                    minorUnits='100'
                                    currency='RUR'
                                    transparentMinor={true}
                                    titleProps={{ color }}
                                />
                            </PureCell.Main>
                            <PureCell.Addon verticalAlign='center'>
                                <ChevronForwardMIcon />
                            </PureCell.Addon>
                        </PureCell.Content>
                    </PureCell>
                </div>
                <div
                    style={{
                        display: 'flex',
                        gap: '20px',
                        flexWrap: 'nowrap',
                        flexDirection: 'column',
                    }}
                >
                    <PureCell
                        href={text('href', '')}
                         verticalPadding={verticalPaddingPrepared}
                        horizontalPadding={horizontalPadding}
                        direction={direction}
                    >
                        <PureCell.Content>
                            <PureCell.Main>
                                <PureCell.Text
                                    titleColor='primary'
                                    view='component-primary'
                                    rowLimit={1}
                                >
                                    Яндекс Лавка
                                </PureCell.Text>
                                <PureCell.Category
                                    categoryName={'Кафе, рестораны, столовые, бары'}
                                    categoryPercent={10}
                                />
                            </PureCell.Main>
                            <PureCell.Footer>
                                <PureCell.Comment>Комментарий есть</PureCell.Comment>
                            </PureCell.Footer>
                        </PureCell.Content>
                    </PureCell>
                    <PureCell
                        href={text('href', '')}
                        verticalPadding={verticalPaddingPrepared}
                        horizontalPadding={horizontalPadding}
                        direction={direction}
                    >
                        <PureCell.Graphics graphicPadding={graphicPadding}>
                            <Circle />
                        </PureCell.Graphics>
                        <PureCell.Content>
                            <PureCell.Main>
                                <PureCell.Text titleColor='primary' view='component-primary'>
                                    Card Name
                                </PureCell.Text>
                                <PureCell.Text titleColor='secondary' view='primary-small'>
                                    Additional Info
                                </PureCell.Text>
                            </PureCell.Main>
                            <PureCell.Addon verticalAlign='center'>
                                <div style={{ width: 24, height: 24, display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                                    <StatusBadge size={20} view='neutral-information' />
                                </div>
                            </PureCell.Addon>
                        </PureCell.Content>
                    </PureCell>
                    <PureCell
                        href={text('href', '')}
                         verticalPadding={verticalPaddingPrepared}
                        horizontalPadding={horizontalPadding}
                        direction={direction}
                    >
                        <PureCell.Graphics graphicPadding={graphicPadding}>
                            <Circle />
                        </PureCell.Graphics>
                        <PureCell.Content>
                            <PureCell.Main>
                                <PureCell.Text
                                    titleColor='primary'
                                    view='component-primary'
                                    rowLimit={1}
                                >
                                    Яндекс Лавка
                                </PureCell.Text>
                                <PureCell.Category
                                    categoryName={'Кафе, рестораны, столовые, бары'}
                                    categoryPercent={10}
                                />
                            </PureCell.Main>
                            <PureCell.Footer>
                                <PureCell.ExtraSubtitle>Status</PureCell.ExtraSubtitle>
                                <PureCell.FooterButton>Button</PureCell.FooterButton>
                            </PureCell.Footer>
                        </PureCell.Content>
                    </PureCell>
                </div>
            </div>
        );
    })}

</Story>

<Story name='PureCell.Addon'>
    {React.createElement(() => {
        const verticalPadding = select(
            'verticalPadding',
            ['airy', 'default', 'compact', 'tiny', 'none'],
            'default',
        );
        const verticalTopPadding = select(
            'verticalPadding.top',
            ['airy', 'default', 'compact', 'tiny', 'none', undefined],
            undefined,
        );
        const verticalBottomPadding = select(
            'verticalPadding.bottom',
            ['airy', 'default', 'compact', 'tiny', 'none', undefined],
            undefined,
        );
        const verticalPaddingPrepared = (verticalTopPadding || verticalBottomPadding) ? {top: verticalTopPadding, bottom: verticalBottomPadding } : verticalPadding

        const horizontalPaddingSide = select(
            'horizontalPadding',
            ['left', 'right', 'both', 'none'],
            'none',
        );
        const horizontalLeftPadding = select('horizontalPadding.left', [0, 16, 20, undefined], undefined);
        const horizontalRightPadding = select('horizontalPadding.right', [0, 16, 20, undefined], undefined);
        const horizontalPadding = (horizontalLeftPadding !== undefined || horizontalRightPadding !== undefined) ? { left: horizontalLeftPadding, right: horizontalRightPadding } : horizontalPaddingSide;
        const graphicPadding = select(
            'graphicPadding',
            ['max', 'airy', 'default', 'compact', 'tiny', 'none', undefined],
            undefined,
        );
        const addonPadding = select('addonPadding', ['default', 'none'], 'default');
        const direction = select('direction', ['vertical', 'horizontal'], 'horizontal');
        return (
            <div
                style={{
                    width: '400px',
                }}
            >
                <PureCell
                    href={text('href', '')}
                    verticalPadding={verticalPaddingPrepared}
                    horizontalPadding={horizontalPadding}
                    direction={direction}
                >
                    <PureCell.Graphics graphicPadding={graphicPadding}>
                        <Circle />
                    </PureCell.Graphics>
                    <PureCell.Content>
                        <PureCell.Main>
                            <PureCell.Text titleColor='primary' view='component-primary'>
                                Card Name
                            </PureCell.Text>
                            <PureCell.Text titleColor='secondary' view='primary-small'>
                                Additional Info
                            </PureCell.Text>
                        </PureCell.Main>
                        <PureCell.Addon verticalAlign='center' addonPadding={addonPadding}>
                            <div style={{ width: 24, height: 24, display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                                <StatusBadge size={20} view='neutral-information' />
                            </div>
                        </PureCell.Addon>
                    </PureCell.Content>
                </PureCell>
            </div>
        );
    })}

</Story>

<Story name='PureCell.Amount'>
    {React.createElement(() => {
        const verticalPadding = select(
            'verticalPadding',
            ['airy', 'default', 'compact', 'tiny', 'none'],
            'default',
        );
        const verticalTopPadding = select(
            'verticalPadding.top',
            ['airy', 'default', 'compact', 'tiny', 'none', undefined],
            undefined,
        );
        const verticalBottomPadding = select(
            'verticalPadding.bottom',
            ['airy', 'default', 'compact', 'tiny', 'none', undefined],
            undefined,
        );
        const verticalPaddingPrepared = (verticalTopPadding || verticalBottomPadding) ? {top: verticalTopPadding, bottom: verticalBottomPadding } : verticalPadding

        const horizontalPaddingSide = select(
            'horizontalPadding',
            ['left', 'right', 'both', 'none'],
            'none',
        );
        const horizontalLeftPadding = select('horizontalPadding.left', [0, 16, 20, undefined], undefined);
        const horizontalRightPadding = select('horizontalPadding.right', [0, 16, 20, undefined], undefined);
        const horizontalPadding = (horizontalLeftPadding !== undefined || horizontalRightPadding !== undefined) ? { left: horizontalLeftPadding, right: horizontalRightPadding } : horizontalPaddingSide;
        const direction = select('direction', ['vertical', 'horizontal'], 'horizontal');
        const weight = select('weight', ['bold', 'normal'], 'normal');
        const view = select('view', ['component', 'primary-small'], 'component');
        const viewAmount = select('viewAmount', ['default', 'withZeroMinorPart'], 'default');
        const color = select(
            'color',
            [
                'tertiary',
                'disabled',
                'accent',
                'primary',
                'attention',
                'positive',
                'secondary',
                'tertiary-inverted',
                'primary-inverted',
                'secondary-inverted',
                'link',
                'negative',
            ],
            'primary',
        );
        const value = text('value', '100000000');
        const hidden = boolean('hidden', false);
        return (
            <PureCell
                href={text('href', '')}
                verticalPadding={verticalPaddingPrepared}
                horizontalPadding={horizontalPadding}
                direction={direction}
            >
                <PureCell.Content>
                    <PureCell.Main>
                        <PureCell.Text titleColor='secondary' view='primary-small'>
                            Product Name ··0000
                        </PureCell.Text>
                        <PureCell.Amount
                            hidden={hidden}
                            view={viewAmount}
                            value={value}
                            minorUnits='100'
                            currency='RUR'
                            weight={weight}
                            color={color}
                        />
                    </PureCell.Main>
                    <PureCell.Footer>
                        <Typography.Text color='secondary' view='primary-small'>
                            Caption Secondary
                        </Typography.Text>
                        <PureCell.FooterButton>Action</PureCell.FooterButton>
                    </PureCell.Footer>
                </PureCell.Content>
            </PureCell>
        );
    })}

</Story>

<Story name='PureCell.AmountTitle'>
    {React.createElement(() => {
                const verticalPadding = select(
            'verticalPadding',
            ['airy', 'default', 'compact', 'tiny', 'none'],
            'default',
        );
        const verticalTopPadding = select(
            'verticalPadding.top',
            ['airy', 'default', 'compact', 'tiny', 'none', undefined],
            undefined,
        );
        const verticalBottomPadding = select(
            'verticalPadding.bottom',
            ['airy', 'default', 'compact', 'tiny', 'none', undefined],
            undefined,
        );
        const verticalPaddingPrepared = (verticalTopPadding || verticalBottomPadding) ? {top: verticalTopPadding, bottom: verticalBottomPadding } : verticalPadding

        const horizontalPaddingSide = select(
            'horizontalPadding',
            ['left', 'right', 'both', 'none'],
            'none',
        );
        const horizontalLeftPadding = select('horizontalPadding.left', [0, 16, 20, undefined], undefined);
        const horizontalRightPadding = select('horizontalPadding.right', [0, 16, 20, undefined], undefined);
        const horizontalPadding = (horizontalLeftPadding !== undefined || horizontalRightPadding !== undefined) ? { left: horizontalLeftPadding, right: horizontalRightPadding } : horizontalPaddingSide;
        const direction = select('direction', ['vertical', 'horizontal'], 'horizontal');
        const viewAmount = select(
            'viewAmount',
            ['default', 'withZeroMinorPart'],
            'withZeroMinorPart',
        );
        const color = select(
            'color',
            [
                'tertiary',
                'disabled',
                'accent',
                'primary',
                'attention',
                'positive',
                'secondary',
                'tertiary-inverted',
                'primary-inverted',
                'secondary-inverted',
                'link',
                'negative',
            ],
            'primary',
        );
        const value = text('value', '100000000');
        const hidden = boolean('hidden', false);

        return (
            <PureCell
                href={text('href', '')}
                verticalPadding={verticalPaddingPrepared}
                horizontalPadding={horizontalPadding}
                direction={direction}
            >
                <PureCell.Content>
                    <PureCell.Main>
                        <PureCell.Text titleColor='secondary' view='primary-small'>
                            Общий баланс
                        </PureCell.Text>
                        <PureCell.AmountTitle
                            hidden={hidden}
                            view={viewAmount}
                            value={value}
                            minorUnits='100'
                            currency='RUR'
                            transparentMinor={true}
                            titleProps={{ color }}
                        />
                    </PureCell.Main>
                    <PureCell.Addon verticalAlign='center'>
                        <ChevronForwardMIcon />
                    </PureCell.Addon>
                </PureCell.Content>
            </PureCell>
        );
    })}

</Story>

<Story name='PureCell.Category'>
    {React.createElement(() => {
        const verticalPadding = select(
            'verticalPadding',
            ['airy', 'default', 'compact', 'tiny', 'none'],
            'default',
        );
        const verticalTopPadding = select(
            'verticalPadding.top',
            ['airy', 'default', 'compact', 'tiny', 'none', undefined],
            undefined,
        );
        const verticalBottomPadding = select(
            'verticalPadding.bottom',
            ['airy', 'default', 'compact', 'tiny', 'none', undefined],
            undefined,
        );
        const verticalPaddingPrepared = (verticalTopPadding || verticalBottomPadding) ? {top: verticalTopPadding, bottom: verticalBottomPadding } : verticalPadding

        const horizontalPaddingSide = select(
            'horizontalPadding',
            ['left', 'right', 'both', 'none'],
            'none',
        );
        const horizontalLeftPadding = select('horizontalPadding.left', [0, 16, 20, undefined], undefined);
        const horizontalRightPadding = select('horizontalPadding.right', [0, 16, 20, undefined], undefined);
        const horizontalPadding = (horizontalLeftPadding !== undefined || horizontalRightPadding !== undefined) ? { left: horizontalLeftPadding, right: horizontalRightPadding } : horizontalPaddingSide;

        const graphicPadding = select(
            'graphicPadding',
            ['airy', 'default', 'compact', 'tiny', 'none', undefined],
            undefined,
        );

        const direction = select('direction', ['vertical', 'horizontal'], 'horizontal');
        const categoryName = text('categoryName', 'Кафе, рестораны, столовые, бары');
        const categoryPercent = text('categoryPercent', '10');
        return (
            <PureCell
                href={text('href', '')}
                verticalPadding={verticalPaddingPrepared}
                horizontalPadding={horizontalPadding}
                direction={direction}
            >
                <PureCell.Graphics graphicPadding={graphicPadding}>
                    <Circle />
                </PureCell.Graphics>
                <PureCell.Content>
                    <PureCell.Main>
                        <PureCell.Text titleColor='primary' view='component-primary' rowLimit={1}>
                            Яндекс Лавка
                        </PureCell.Text>
                        <PureCell.Category
                            categoryName={categoryName}
                            categoryPercent={categoryPercent}
                        />
                    </PureCell.Main>
                </PureCell.Content>
            </PureCell>
        );
    })}

</Story>

<Story name='PureCell.Comment'>
    {React.createElement(() => {
        const verticalPadding = select(
            'verticalPadding',
            ['airy', 'default', 'compact', 'tiny', 'none'],
            'default',
        );
        const verticalTopPadding = select(
            'verticalPadding.top',
            ['airy', 'default', 'compact', 'tiny', 'none', undefined],
            undefined,
        );
        const verticalBottomPadding = select(
            'verticalPadding.bottom',
            ['airy', 'default', 'compact', 'tiny', 'none', undefined],
            undefined,
        );
        const verticalPaddingPrepared = (verticalTopPadding || verticalBottomPadding) ? {top: verticalTopPadding, bottom: verticalBottomPadding } : verticalPadding

        const horizontalPaddingSide = select(
            'horizontalPadding',
            ['left', 'right', 'both', 'none'],
            'none',
        );
        const horizontalLeftPadding = select('horizontalPadding.left', [0, 16, 20, undefined], undefined);
        const horizontalRightPadding = select('horizontalPadding.right', [0, 16, 20, undefined], undefined);
        const horizontalPadding = (horizontalLeftPadding !== undefined || horizontalRightPadding !== undefined) ? { left: horizontalLeftPadding, right: horizontalRightPadding } : horizontalPaddingSide;
        const direction = select('direction', ['vertical', 'horizontal'], 'horizontal');
        return (
            <PureCell
                href={text('href', '')}
                verticalPadding={verticalPaddingPrepared}
                horizontalPadding={horizontalPadding}
                direction={direction}
            >
                <PureCell.Content>
                    <PureCell.Main>
                        <PureCell.Text titleColor='primary' view='component-primary' rowLimit={1}>
                            Яндекс Лавка
                        </PureCell.Text>
                        <PureCell.Category
                            categoryName={'Кафе, рестораны, столовые, бары'}
                            categoryPercent={10}
                        />
                    </PureCell.Main>
                    <PureCell.Footer>
                        <PureCell.Comment>Комментарий есть</PureCell.Comment>
                    </PureCell.Footer>
                </PureCell.Content>
            </PureCell>
        );
    })}

</Story>

<Story name='PureCell.Content'>
    {React.createElement(() => {
                const verticalPadding = select(
            'verticalPadding',
            ['airy', 'default', 'compact', 'tiny', 'none'],
            'default',
        );
        const verticalTopPadding = select(
            'verticalPadding.top',
            ['airy', 'default', 'compact', 'tiny', 'none', undefined],
            undefined,
        );
        const verticalBottomPadding = select(
            'verticalPadding.bottom',
            ['airy', 'default', 'compact', 'tiny', 'none', undefined],
            undefined,
        );
        const verticalPaddingPrepared = (verticalTopPadding || verticalBottomPadding) ? {top: verticalTopPadding, bottom: verticalBottomPadding } : verticalPadding

        const horizontalPaddingSide = select(
            'horizontalPadding',
            ['left', 'right', 'both', 'none'],
            'none',
        );
        const horizontalLeftPadding = select('horizontalPadding.left', [0, 16, 20, undefined], undefined);
        const horizontalRightPadding = select('horizontalPadding.right', [0, 16, 20, undefined], undefined);
        const horizontalPadding = (horizontalLeftPadding !== undefined || horizontalRightPadding !== undefined) ? { left: horizontalLeftPadding, right: horizontalRightPadding } : horizontalPaddingSide;
        const graphicPadding = select(
            'graphicPadding',
            ['airy', 'default', 'compact', 'tiny', 'none', undefined],
            undefined,
        );
        const direction = select('direction', ['vertical', 'horizontal'], 'horizontal');
        return (
            <div
                style={{
                    width: '400px',
                }}
            >
                <PureCell
                    href={text('href', '')}
                    verticalPadding={verticalPaddingPrepared}
                    horizontalPadding={horizontalPadding}
                    direction={direction}
                >
                    <PureCell.Graphics graphicPadding={graphicPadding}>
                        <Circle />
                    </PureCell.Graphics>
                    <PureCell.Content>
                        <PureCell.Main>
                            <PureCell.Text
                                titleColor='primary'
                                view='component-primary'
                                rowLimit={1}
                            >
                                Яндекс Лавка
                            </PureCell.Text>
                            <PureCell.Category
                                categoryName={'Кафе, рестораны, столовые, бары'}
                                categoryPercent={10}
                            />
                        </PureCell.Main>
                        <PureCell.Addon>
                            <div style={{ width: 24, height: 24, display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                                <StatusBadge size={20} view='neutral-information' />
                            </div>
                        </PureCell.Addon>
                    </PureCell.Content>
                </PureCell>
            </div>
        );
    })}

</Story>

<Story name='PureCell.Footer'>
    {React.createElement(() => {
        const footerPadding = select('footerPadding', ['default', 'none'], 'default');
        const verticalPadding = select(
            'verticalPadding',
            ['airy', 'default', 'compact', 'tiny', 'none'],
            'default',
        );
        const verticalTopPadding = select(
            'verticalPadding.top',
            ['airy', 'default', 'compact', 'tiny', 'none', undefined],
            undefined,
        );
        const verticalBottomPadding = select(
            'verticalPadding.bottom',
            ['airy', 'default', 'compact', 'tiny', 'none', undefined],
            undefined,
        );
        const verticalPaddingPrepared = (verticalTopPadding || verticalBottomPadding) ? {top: verticalTopPadding, bottom: verticalBottomPadding } : verticalPadding

        const horizontalPaddingSide = select(
            'horizontalPadding',
            ['left', 'right', 'both', 'none'],
            'none',
        );
        const horizontalLeftPadding = select('horizontalPadding.left', [0, 16, 20, undefined], undefined);
        const horizontalRightPadding = select('horizontalPadding.right', [0, 16, 20, undefined], undefined);
        const horizontalPadding = (horizontalLeftPadding !== undefined || horizontalRightPadding !== undefined) ? { left: horizontalLeftPadding, right: horizontalRightPadding } : horizontalPaddingSide;
        const graphicPadding = select(
            'graphicPadding',
            ['airy', 'default', 'compact', 'tiny', 'none', undefined],
            undefined,
        );
        const direction = select('direction', ['vertical', 'horizontal'], 'horizontal');
        return (
            <PureCell
                href={text('href', '')}
                verticalPadding={verticalPaddingPrepared}
                horizontalPadding={horizontalPadding}
                direction={direction}
            >
                <PureCell.Graphics graphicPadding={graphicPadding}>
                    <Circle />
                </PureCell.Graphics>
                <PureCell.Content>
                    <PureCell.Main>
                        <PureCell.Text titleColor='primary' view='component-primary' rowLimit={1}>
                            Яндекс Лавка
                        </PureCell.Text>
                        <PureCell.Category
                            categoryName={'Кафе, рестораны, столовые, бары'}
                            categoryPercent={10}
                        />
                    </PureCell.Main>
                    <PureCell.Footer footerPadding={footerPadding}>
                        <PureCell.ExtraSubtitle>Status</PureCell.ExtraSubtitle>
                        <PureCell.FooterButton>Button</PureCell.FooterButton>
                    </PureCell.Footer>
                </PureCell.Content>
            </PureCell>
        );
    })}

</Story>

<Story name='PureCell.FooterButton'>
    {React.createElement(() => {
        const footerPadding = select('footerPadding', ['default', 'none'], 'default');
        const verticalPadding = select(
            'verticalPadding',
            ['airy', 'default', 'compact', 'tiny', 'none'],
            'default',
        );
        const verticalTopPadding = select(
            'verticalPadding.top',
            ['airy', 'default', 'compact', 'tiny', 'none', undefined],
            undefined,
        );
        const verticalBottomPadding = select(
            'verticalPadding.bottom',
            ['airy', 'default', 'compact', 'tiny', 'none', undefined],
            undefined,
        );
        const verticalPaddingPrepared = (verticalTopPadding || verticalBottomPadding) ? {top: verticalTopPadding, bottom: verticalBottomPadding } : verticalPadding

        const horizontalPaddingSide = select(
            'horizontalPadding',
            ['left', 'right', 'both', 'none'],
            'none',
        );
        const horizontalLeftPadding = select('horizontalPadding.left', [0, 16, 20, undefined], undefined);
        const horizontalRightPadding = select('horizontalPadding.right', [0, 16, 20, undefined], undefined);
        const horizontalPadding = (horizontalLeftPadding !== undefined || horizontalRightPadding !== undefined) ? { left: horizontalLeftPadding, right: horizontalRightPadding } : horizontalPaddingSide;
        const direction = select('direction', ['vertical', 'horizontal'], 'horizontal');
        const colors = select('colors', ['default', 'inverted'], 'default');
        return (
            <PureCell
                href={text('href', '')}
                verticalPadding={verticalPaddingPrepared}
                horizontalPadding={horizontalPadding}
                direction={direction}
            >
                <PureCell.Content>
                    <PureCell.Main>
                        <PureCell.Text titleColor='primary' view='component-primary' rowLimit={1}>
                            Яндекс Лавка
                        </PureCell.Text>
                        <PureCell.Category
                            categoryName={'Кафе, рестораны, столовые, бары'}
                            categoryPercent={10}
                        />
                    </PureCell.Main>
                    <PureCell.Footer footerPadding={footerPadding}>
                        <PureCell.FooterButton
                            view={select('view', VIEWS, 'primary')}
                            colors={colors}
                            href={text('href', '')}
                            loading={boolean('loading')}
                            disabled={boolean('disabled', false)}
                            block={boolean('block', false)}
                            nowrap={boolean('nowrap', false)}
                            leftAddons={boolean('leftAddons', false) && <StarMIcon />}
                            rightAddons={boolean('rightAddons', false) && <StarMIcon />}
                        >
                            Button
                        </PureCell.FooterButton>
                    </PureCell.Footer>
                </PureCell.Content>
            </PureCell>
        );
    })}

</Story>

<Story name='PureCell.ExtraSubtitle'>
    {React.createElement(() => {
        const footerPadding = select('footerPadding', ['default', 'none'], 'default');
        const verticalPadding = select(
            'verticalPadding',
            ['airy', 'default', 'compact', 'tiny', 'none'],
            'default',
        );
        const verticalTopPadding = select(
            'verticalPadding.top',
            ['airy', 'default', 'compact', 'tiny', 'none', undefined],
            undefined,
        );
        const verticalBottomPadding = select(
            'verticalPadding.bottom',
            ['airy', 'default', 'compact', 'tiny', 'none', undefined],
            undefined,
        );
        const verticalPaddingPrepared = (verticalTopPadding || verticalBottomPadding) ? {top: verticalTopPadding, bottom: verticalBottomPadding } : verticalPadding

        const horizontalPaddingSide = select(
            'horizontalPadding',
            ['left', 'right', 'both', 'none'],
            'none',
        );
        const horizontalLeftPadding = select('horizontalPadding.left', [0, 16, 20, undefined], undefined);
        const horizontalRightPadding = select('horizontalPadding.right', [0, 16, 20, undefined], undefined);
        const horizontalPadding = (horizontalLeftPadding !== undefined || horizontalRightPadding !== undefined) ? { left: horizontalLeftPadding, right: horizontalRightPadding } : horizontalPaddingSide;
        const direction = select('direction', ['vertical', 'horizontal'], 'horizontal');
        const color = select(
            'color',
            [
                'tertiary',
                'disabled',
                'accent',
                'primary',
                'attention',
                'positive',
                'secondary',
                'tertiary-inverted',
                'primary-inverted',
                'secondary-inverted',
                'link',
                'negative',
            ],
            'secondary',
        );
        return (
            <PureCell
                href={text('href', '')}
                verticalPadding={verticalPaddingPrepared}
                horizontalPadding={horizontalPadding}
                direction={direction}
            >
                <PureCell.Content>
                    <PureCell.Main>
                        <PureCell.Text titleColor='primary' view='component-primary' rowLimit={1}>
                            Яндекс Лавка
                        </PureCell.Text>
                        <PureCell.Category
                            categoryName={'Кафе, рестораны, столовые, бары'}
                            categoryPercent={10}
                        />
                    </PureCell.Main>
                    <PureCell.Footer footerPadding={footerPadding}>
                        <PureCell.ExtraSubtitle color={color}>Status</PureCell.ExtraSubtitle>
                    </PureCell.Footer>
                </PureCell.Content>
            </PureCell>
        );
    })}

</Story>

<Story name='PureCell.Graphics'>
    {React.createElement(() => {
        const verticalPadding = select(
            'verticalPadding',
            ['airy', 'default', 'compact', 'tiny', 'none'],
            'default',
        );
        const verticalTopPadding = select(
            'verticalPadding.top',
            ['airy', 'default', 'compact', 'tiny', 'none', undefined],
            undefined,
        );
        const verticalBottomPadding = select(
            'verticalPadding.bottom',
            ['airy', 'default', 'compact', 'tiny', 'none', undefined],
            undefined,
        );
        const verticalPaddingPrepared = (verticalTopPadding || verticalBottomPadding) ? {top: verticalTopPadding, bottom: verticalBottomPadding } : verticalPadding

        const horizontalPaddingSide = select(
            'horizontalPadding',
            ['left', 'right', 'both', 'none'],
            'none',
        );
        const horizontalLeftPadding = select('horizontalPadding.left', [0, 16, 20, undefined], undefined);
        const horizontalRightPadding = select('horizontalPadding.right', [0, 16, 20, undefined], undefined);
        const horizontalPadding = (horizontalLeftPadding !== undefined || horizontalRightPadding !== undefined) ? { left: horizontalLeftPadding, right: horizontalRightPadding } : horizontalPaddingSide;
        const graphicPadding = select(
            'graphicPadding',
            ['airy', 'default', 'compact', 'tiny', 'none', undefined],
            undefined,
        );
        const direction = select('direction', ['vertical', 'horizontal'], 'horizontal');
        return (
            <PureCell
                href={text('href', '')}
                verticalPadding={verticalPaddingPrepared}
                horizontalPadding={horizontalPadding}
                direction={direction}
            >
                <PureCell.Graphics graphicPadding={graphicPadding}>
                    <Circle />
                </PureCell.Graphics>
            </PureCell>
        );
    })}

</Story>

<Story name='PureCell.Main'>
    {React.createElement(() => {
        const previewStyles = stylesStringToObj(getQueryParam('wrapperStyles'));
        const isPreview = Object.keys(previewStyles).length > 0;
        const verticalPadding = select(
            'verticalPadding',
            ['airy', 'default', 'compact', 'tiny', 'none'],
            'default',
        );
        const verticalTopPadding = select(
            'verticalPadding.top',
            ['airy', 'default', 'compact', 'tiny', 'none', undefined],
            undefined,
        );
        const verticalBottomPadding = select(
            'verticalPadding.bottom',
            ['airy', 'default', 'compact', 'tiny', 'none', undefined],
            undefined,
        );
        const verticalPaddingPrepared = (verticalTopPadding || verticalBottomPadding) ? {top: verticalTopPadding, bottom: verticalBottomPadding } : verticalPadding

        const horizontalPaddingSide = select(
            'horizontalPadding',
            ['left', 'right', 'both', 'none'],
            'none',
        );
        const horizontalLeftPadding = select('horizontalPadding.left', [0, 16, 20, undefined], undefined);
        const horizontalRightPadding = select('horizontalPadding.right', [0, 16, 20, undefined], undefined);
        const horizontalPadding = (horizontalLeftPadding !== undefined || horizontalRightPadding !== undefined) ? { left: horizontalLeftPadding, right: horizontalRightPadding } : horizontalPaddingSide;
        const graphicPadding = select(
            'graphicPadding',
            ['airy', 'default', 'compact', 'tiny', 'none', undefined],
            undefined,
        );
        const direction = select('direction', ['vertical', 'horizontal'], 'horizontal');
        const styleAddons = {
            width: '40px',
            height: '40px',
            borderRadius: '8px',
            backgroundColor: 'rgba(55, 120, 251, 0.1)',
            border: '1px dashed rgb(55, 120, 251)',
            boxSizing: 'border-box',
        };
        const styleFooter = {
            width: '240px',
            height: '16px',
            borderRadius: '8px',
            backgroundColor: 'rgba(55, 120, 251, 0.1)',
            border: '1px dashed rgb(55, 120, 251)',
            boxSizing: 'border-box',
        };
        const styleContent = {
            width: '180px',
            height: '20px',
            borderRadius: '8px',
            backgroundColor: 'rgba(55, 120, 251, 0.1)',
            border: '1px dashed rgb(55, 120, 251)',
            boxSizing: 'border-box',
        };
        const styleGraphics = {
            width: '64px',
            height: '64px',
            borderRadius: '8px',
            backgroundColor: 'rgba(55, 120, 251, 0.1)',
            border: '1px dashed rgb(55, 120, 251)',
            boxSizing: 'border-box',
        };
        return isPreview ? (
            <div style={previewStyles}>
                <div
                    style={{
                        width: '320px',
                        backgroundColor: 'rgba(55, 120, 251, 0.1)',
                        padding: '12px 16px',
                    }}
                >
                    <PureCell padding='default' direction='horizontal'>
                        <PureCell.Graphics verticalAlign='center'>
                            <div style={styleGraphics} />
                        </PureCell.Graphics>
                        <PureCell.Content>
                            <PureCell.Main>
                                <div style={styleContent} />
                                <div style={styleContent} />
                            </PureCell.Main>
                            <PureCell.Addon verticalAlign='center'>
                                <div style={styleAddons} />
                            </PureCell.Addon>
                            <PureCell.Footer footerPadding='default'>
                                <div style={styleFooter} />
                            </PureCell.Footer>
                        </PureCell.Content>
                    </PureCell>
                </div>
            </div>
        ) : (
            <PureCell
                href={text('href', '')}
                verticalPadding={verticalPaddingPrepared}
                horizontalPadding={horizontalPadding}
                direction={direction}
            >
                <PureCell.Graphics graphicPadding={graphicPadding}>
                    <Circle />
                </PureCell.Graphics>
                <PureCell.Content>
                    <PureCell.Main>
                        <PureCell.Text titleColor='primary' view='component-primary' rowLimit={1}>
                            Яндекс Лавка
                        </PureCell.Text>
                        <PureCell.Category
                            categoryName={'Кафе, рестораны, столовые, бары'}
                            categoryPercent={10}
                        />
                    </PureCell.Main>
                    <PureCell.Addon></PureCell.Addon>
                    <PureCell.Footer>
                        <PureCell.Comment>Комментарий есть</PureCell.Comment>
                    </PureCell.Footer>
                </PureCell.Content>
            </PureCell>
        );
    })}

</Story>

<Story name='PureCell.Text'>
    {React.createElement(() => {
        const verticalPadding = select(
            'verticalPadding',
            ['airy', 'default', 'compact', 'tiny', 'none'],
            'default',
        );
        const verticalTopPadding = select(
            'verticalPadding.top',
            ['airy', 'default', 'compact', 'tiny', 'none', undefined],
            undefined,
        );
        const verticalBottomPadding = select(
            'verticalPadding.bottom',
            ['airy', 'default', 'compact', 'tiny', 'none', undefined],
            undefined,
        );
        const verticalPaddingPrepared = (verticalTopPadding || verticalBottomPadding) ? {top: verticalTopPadding, bottom: verticalBottomPadding } : verticalPadding

        const horizontalPaddingSide = select(
            'horizontalPadding',
            ['left', 'right', 'both', 'none'],
            'none',
        );
        const horizontalLeftPadding = select('horizontalPadding.left', [0, 16, 20, undefined], undefined);
        const horizontalRightPadding = select('horizontalPadding.right', [0, 16, 20, undefined], undefined);
        const horizontalPadding = (horizontalLeftPadding !== undefined || horizontalRightPadding !== undefined) ? { left: horizontalLeftPadding, right: horizontalRightPadding } : horizontalPaddingSide;
        const graphicPadding = select(
            'graphicPadding',
            ['airy', 'default', 'compact', 'tiny', 'none', undefined],
            undefined,
        );
        const direction = select('direction', ['vertical', 'horizontal'], 'horizontal');
        const rowLimit = select('rowLimit', ['1', '2'], '1');
        const view = select('view', ['primary-large', 'primary-medium', 'primary-small', 'component', 'component-primary', 'component-secondary', 'secondary-large'], 'component');
        const color = select(
            'titleColor',
            [
                'tertiary',
                'disabled',
                'accent',
                'primary',
                'attention',
                'positive',
                'secondary',
                'tertiary-inverted',
                'primary-inverted',
                'secondary-inverted',
                'link',
                'negative',
            ],
            'primary',
        );
        return (
            <PureCell
                href={text('href', '')}
                verticalPadding={verticalPaddingPrepared}
                horizontalPadding={horizontalPadding}
                direction={direction}
            >
                <PureCell.Graphics graphicPadding={graphicPadding}>
                    <Circle />
                </PureCell.Graphics>
                <PureCell.Content>
                    <PureCell.Main>
                        <PureCell.Text
                            titleColor={color}
                            value={text('value', '')}
                            view={view}
                            rowLimit={rowLimit}
                        >
                            Яндекс Лавка
                        </PureCell.Text>
                        <PureCell.Category
                            categoryName={'Кафе, рестораны, столовые, бары'}
                            categoryPercent={10}
                        />
                    </PureCell.Main>
                    <PureCell.Footer>
                        <PureCell.Comment>Комментарий есть</PureCell.Comment>
                    </PureCell.Footer>
                </PureCell.Content>
            </PureCell>
        );
    })}

</Story>

{/* Docs */}

<ComponentHeader name='PureCell' children='Многофункциональная ячейка.' />

<Tabs
    description={<Description />}
    development={<Development />}
    changelog={<Markdown>{Changelog}</Markdown>}
/>
