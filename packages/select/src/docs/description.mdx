...
По умолчанию селект вызывает [Popover](?path=/docs/components-popover--docs) с вариантами выбора.
На мобильных устройствах варианты выбора отображаются в [BottomSheet](?path=/docs/components-bottomsheet--docs).
...
открывать его в [Modal](?path=/docs/components-modal--docs)
вместо [BottomSheet](?path=/docs/components-bottomsheet--docs).
...
На основе селекта построены такие компоненты как [PickerButton](?path=/docs/components-pickerbutton--docs) и
[InputAutocomplete](?path=/docs/components-inputautocomplete--docs).

## Морфология поля ввода

Компонент является оберткой над [FormControl](?path=/docs/components-formcontrol--docs), поэтому наследует все доступные для него настройки.
...
Для кастомных списков можно использовать ячейку [PureCell](?path=/docs/components-purecell--docs) с расширением в виде контрола.
Благодаря гибкой настройке [PureCell](?path=/docs/components-purecell--docs) можно получить необходимую конфигурацию элементов списка.
