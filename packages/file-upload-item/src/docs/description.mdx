...
render(() => {
    ...
    const startUpload = (finalStatus, errorMessage) => {
        setError('');
        setProgressBar(0);
        setStatus(isProgressKnown ? 'UPLOADING' : 'LOADING');
        ...
        if (isProgressKnown) {
            ...
            // Для режима без прогресса показываем загрузку дольше
            setTimeout(() => {
                setStatus(finalStatus);
                if (errorMessage) setError(errorMessage);
            }, 5000);
    ...
    const handleSuccess = () => startUpload('SUCCESS');
    const handleError = () => startUpload('ERROR', 'Описание первой ошибки');
    const handleMultiError = () =>
        startUpload('ERROR', ['Описание первой ошибки', 'Описание второй ошибки']);
    ...
    const title = isUploading ? 'Пример.docx' : 'Прикрепите файл';
    ...
    return (
        <React.Fragment>
            ...
            <Gap size={20} />
            <div>
                <Button size={32} onClick={handleSuccess}>
                    Успех
                </Button>
