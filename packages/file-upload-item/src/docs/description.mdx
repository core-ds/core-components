...
render(() => {
    ...
    const [isSuccessClicked, setIsSuccessClicked] = React.useState(false);
    ...
    return (
        <React.Fragment>
            <FileUploadItem
                ...
                uploadStatus={status}
                progressBar={isProgressKnown && isSuccessClicked ? progressBar : undefined}
            ...
            <div>
                <Button
                    size={32}
                    onClick={isProgressKnown ? handleSuccessClick : handleLoaderAnimation}
                >
                    Успех
                </Button>
