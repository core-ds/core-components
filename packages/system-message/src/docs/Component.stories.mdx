                    ...
                    >
                        ...
                        {controls && (
                            <SystemMessage.Controls direction={controlDirection}>
                                {showPrimaryButton && (
                                    <Button size={document.body.clientWidth < 371 ? 'm' : 's'} view='primary' breakpoint={breakpoint}>
                                        Получить код
                                    </Button>
                                )}
                                {showSecondaryButton && <Button size={document.body.clientWidth < 371 ? 'm' : 's'} breakpoint={breakpoint}>Отмена</Button>}
