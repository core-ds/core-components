...
Компонент является оберткой над [FormControl](?path=/docs/components-formcontrol--docs),
поэтому наследует все доступные для него настройки.
Список это кастомизируемый [Popover](?path=/docs/components-popover--docs)
в десктопной версии и [Bottomsheet](?path=/docs/components-bottomsheet--docs)
или [Modal](?path=/docs/components-modal--docs)
в мобильной.
