...
render(() => {
    ...
    return (
        <InputAutocompleteMobile
            ...
            Option={CardOption}
            Input={MaskedInput}
            bottomSheetHeaderAddonsProps={{ mask }}
        />
    );
});
```
...
Компонент является оберткой над [FormControl](?path=/docs/formcontrol--form-control),
поэтому наследует все доступные для него настройки.
Список это кастомизируемый [Popover](?path=/docs/popover--popover)
в десктопной версии и [Bottomsheet](?path=/docs/bottomsheet--bottom-sheet)
или [Modal](?path=/docs/modal--modal)
в мобильной.
