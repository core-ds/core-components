...
Скелетон может подстраиваться под заданный размер и форму шейпа компонента.

```jsx live
render(() => {
    const [component, setComponent] = React.useState('SuperEllipse');
    const [isSkeleton, setIsSkeleton] = React.useState(false);
    const [skeletonAnimate, setSkeletonAnimate] = React.useState(false);
    ...
    const onChangeSkeleton = (_, payload) => {
        setIsSkeleton((prevValue) => (payload.checked));

        if (isSkeleton) {
           setSkeletonAnimate(false)
        }
    };

    const onChangeAnimate = (_, payload) => {
        setSkeletonAnimate((prevValue) => (payload.checked ));
    };
            ...
            <Row>
                <Col>
                    <Switch
                        ...
                        onChange={onChangeSkeleton}
                    />
                    <Gap size={12} />
                    <Switch
                        block={true}
                        checked={skeletonAnimate}
                        label='Анимация скелетона'
