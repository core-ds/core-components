import { ArgsTabs, CssVars } from 'storybook/blocks';
import { ModalMobile } from '../mobile';
import { ModalDesktop } from '../desktop';
import { ModalResponsive } from '../Component.responsive';
import vars from '!!raw-loader!../vars.css';

## Подключение

```jsx
// Компаунд импорты
import { Modal } from '@balafla/core-components/modal';
import { ModalDesktop } from '@balafla/core-components/modal/desktop';
import { ModalMobile } from '@balafla/core-components/modal/mobile';

// Контекст
import { ModalContext } from '@balafla/core-components/modal/shared';

// Атомарные импорты
import { ModalComponentDesktop, ContentDesktop, HeaderDesktop, FooterDesktop, ControlsDesktop } from '@balafla/core-components/modal/desktop';
import { ModalComponentMobile, ContentMobile, HeaderMobile, FooterMobile, ControlsMobile } from '@balafla/core-components/modal/mobile';
import { ModalComponentResponsive, ContentResponsive, HeaderResponsive, FooterResponsive, ControlsResponsive } from '@balafla/core-components/modal';

```

Импорт Modal - респонсивный компаунд

## Использование контекста

Компонент модального окна предоставляет доступ к контексту, что позволяет гибко настраивать и расширять компонент, а также упрощает использование.

```tsx
export type ModalContext = {
    // Указывает, используется ли футер
    hasFooter?: boolean;
    // Указывает, используется ли шапка (например, позволяет гибко настраивать отступы)
    hasHeader?: boolean;
    // Указывает наличие скролла
    hasScroll?: boolean;
    // Указывает, что контент прокручен
    headerHighlighted?: boolean;
    // Указывает, что контент прокручен не до конца
    footerHighlighted?: boolean;
    // Коллбэк, который должна вызвать кастомная шапка
    setHasHeader: (exists: boolean) => void;
    // Коллбэк, который должна вызвать кастомный футер
    setHasFooter: (exists: boolean) => void;
    // Позволяет закрыть модальное окно
    onClose: (event: React.MouseEvent<HTMLElement>) => void;
    // Реф, который должен быть установлен контентой области
    contentRef: React.Ref<HTMLElement>;
};
```

## Использование dataTestId

В компоненте используются модификаторы для `dataTestId`.
Для удобного поиска элементов можно воспользоваться функцией `getModalTestIds`.
Импорт из `@balafla/core-components/modal/shared`.

Функция возвращает объект:

```jsx
{
    modal: dataTestId,
    content: `${dataTestId}-content`,
    footer: `${dataTestId}-footer`,
    controls: `${dataTestId}-controls`,
    header: `${dataTestId}-header`,
    title: `${dataTestId}-header-title`,
    closer: `${dataTestId}-header-closer`,
    backButton: `${dataTestId}-header-back-button`,
};
```

## Свойства

<ArgsTabs
    components={{
        ModalDesktop,
        ModalMobile,
        ModalResponsive,
        'Modal.Header': ModalResponsive.Header,
        'Modal.Content': ModalResponsive.Content,
        'Modal.Footer': ModalResponsive.Footer,
        'Modal.Controls': ModalResponsive.Controls,
    }}
/>

## Переменные

<CssVars css={vars} />
