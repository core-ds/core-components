...
### Адаптивная версия модального окна

```jsx live
function Form() {
    const [form, setForm] = React.useState({ name: '', lastName: '' });
...
function Modal() {
    ...
    return (
        <>
            <ModalResponsive onClose={() => setOpen(false)} open={open}>
                <ModalResponsive.Header>Заполните форму</ModalResponsive.Header>
