...
- **full** — без `version`, `v1` и `v2`: весь changelog текущей мажорной версии.
...
Записи, содержащие только обновления зависимостей (без изменений самого компонента), исключены из данных.

| Параметр    | Тип      | Обязательный | Описание                                                 |
| ----------- | -------- | ------------ | -------------------------------------------------------- |
| `component` | `string` | да           | Имя компонента (например `Button`, `Input`, `ActionButton`) |
| `version`   | `string` | нет          | Конкретная версия для запроса, например `1.1.0`          |
| `v1`        | `string` | нет          | Начальная версия для diff-режима, например `1.0.0`       |
| `v2`        | `string` | нет          | Конечная версия для diff-режима, например `1.1.2`        |
