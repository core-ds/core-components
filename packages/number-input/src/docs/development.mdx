import { ArgTypes } from '@storybook/addon-docs';
import { NumberInputResponsive } from '../Component.responsive';

## Подключение

```jsx
// responsive версия компонента
import { NumberInput } from '@balafla/core-components/number-input';

// desktop версия компонента
import { NumberInputDesktop } from '@balafla/core-components/number-input/desktop';

// mobile версия компонента
import { NumberInputMobile } from '@balafla/core-components/number-input/mobile';
```

## Использование dataTestId

В компоненте используются модификаторы для `dataTestId`.
Компонент является оберткой над FormControl, поэтому наследует все его модификаторы.
Для удобного поиска элементов можно воспользоваться функцией `getNumberInputTestIds`.
Импорт из `@balafla/core-components/number-input/shared`.

Функция возвращает объект:

```jsx
{
    input: dataTestId,
    inputWrapper: `${dataTestId}-form-control`,
    inputWrapperInner: `${dataTestId}-form-control-inner`,
    leftAddons: `${dataTestId}-form-control-left-addons`,
    rightAddons: `${dataTestId}-form-control-right-addons`,
    error: `${dataTestId}-form-control-error-message`,
    hint: `${dataTestId}-form-control-hint`,
    decrementButton: `${dataTestId}-decrement-button`,
    incrementButton: `${dataTestId}-increment-button`,
};
```

## Свойства

<ArgTypes of={NumberInputResponsive} />
