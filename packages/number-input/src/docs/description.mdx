...
render(() => {
    ...
    const [minorityRadioValue, setMinorityRadioValue] = React.useState('notLimited');
    const [signsRadioValue, setSignsRadioValue] = React.useState('allowSigns');

    
    const handleChange = (e, paylaod) => {
        setValue(paylaod.value)
    }
    ...
    const handleMinorityRadioChange = React.useCallback((_, payload) => {
        setMinorityRadioValue(payload.value);
    }, []);
    ...
    return (
        <div style={{ width: document.body.clientWidth < 450 ? '100%' : 320 }}>
            <NumberInput
                ...
                label='Label'
                signs={allowSignsRadioValue === 'comma' ? ',' : '.'}
                minority={minorityRadioValue === 'limited' ? 2 : null}
            ...
            <div style={{ margin: "40px 0" }}>
                ...
                onChange={handleMinorityRadioChange}
                value={minorityRadioValue}
                >
...
Поле ввода является оберткой над [FormControl](/docs/components-forms-inputs-formcontrol--form-control), поэтому наследует все доступные для него настройки.