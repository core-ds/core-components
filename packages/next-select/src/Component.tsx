...
import { ListRefType, RenderItemExtraProps } from './components/list';
...
const useElementWidth = createUseMeasure((div: HTMLDivElement) => {
...
function SelectRenderFunction<ValueType extends BaseValueType>(
    ...
    const optionsListRef = useRef<ListRefType>(null);
    const [rootElementWidth, rootElementRef] = useElementWidth();
    ...
    const renderOption = (
        ...
        if (isGroupOption(option)) {
            ...
            return (
                <div
                    {...optionGroupProps}
                    ref={internalMergeRefs([optionGroupProps.ref])}
                    key={optionGroupProps.key}
                >
                    {optionLabel}
                </div>
            );
