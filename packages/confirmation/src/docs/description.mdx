...
4. Закончились попытки ввода кода: после ввода кода пользователю будет сообщено, что у него закончились попытки.
5. Закончились попытки запроса кода: после запроса следующего кода запрос нового будет заблокирован.
...
render(() => {
    ...
    const variants = [
        ...
        { key: 'temp-block-over', content: 'Сценарий нового экрана, когда форма временно заблокирована' },
    ];
