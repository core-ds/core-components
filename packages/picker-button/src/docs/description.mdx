...
PickerButton это [Select](/docs/components-forms-selects-select--select) с [Button](/docs/components-actions-button--button) вместо стандартного `field`.
Поэтому PickerButton доступен во всех `size` и `view`, которые есть у [Button](/docs/components-actions-button--button).
