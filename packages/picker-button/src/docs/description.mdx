...
render(
    <Space>
        ...
        <PickerButtonMobile options={options} view='primary' label='Mobile' />
        <PickerButtonDesktop options={options} view='primary' label='Responsive' />
    </Space>,
);
```
...
PickerButton это [Select](path=/docs/компоненты-селекты-select--select) с [Button](path=/docs/компоненты-кнопки-button--button) вместо стандартного `field`.
Поэтому PickerButton доступен во всех `size` и `view`, которые есть у [Button](path=/docs/компоненты-кнопки-button--button).
