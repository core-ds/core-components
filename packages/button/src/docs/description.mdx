...
## Ширина 
Кнопка адаптируется под длину контента. Для каждого вертигального размера кнопки задан минимальный горизонтальный размер. 
...
render(() => {
    ...
    return (
        <Container>
            <Row>
                <Col>
                    <Button leftAddons={leftAddons && <StarMIcon/>} rightAddons={rightAddons && <StarMIcon/>} view="primary">
                        {label && 'Label'}
                    </Button>
                </Col>
            </Row>
            ...
            <Row align='middle'>
                <Switch label={<span>LeftAddons</span>} checked={leftAddons} onChange={handleLeftAddonsChange} />
            </Row>

            <Row align='middle'>
                <Switch label={<span>RightAddons</span>} checked={rightAddons} onChange={handleRightAddonsChange} />
            </Row>
...
render(() => {
    ...
    return (
        ...
        <Row align='middle'>
            <Switch label={<span>Запретить перенос строки</span>} checked={checked} onChange={handleChange} />
        </Row>
    </Container>
    );
});
...
Если нужна кнопка с одной иконкой, но без подложки, используйте <a href="/docs/компоненты-iconbutton--icon-button">IconButton</a>

Если нужна кнопка с другим цветом фона, используйте <a href="/docs/компоненты-custombutton--custom-button">CustomButton</a>.

Если нужна кнопка с выпадающим списком, используйте <a href="/docs/компоненты-pickerbutton--picker-button">PickerButton</a>.
