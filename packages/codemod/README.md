...
| 49-input-show-error-icon | Добавляет свойство `showErrorIcon` со значением `true` компоненту `Input` и всем компонентам которые наследуют от него отображение ошибки. Данный кодмод предназначен для темы `site`                                               |
