...
render(() => {
    ...
    const css = `
        button[data-test-id='icon'] > span > span {
...
render(() => {
    ...
    const css = `
       button[data-test-id='icon-left'] > span > span {
            justify-content: end;
        }

        button[data-test-id='icon-right'] > span > span {
