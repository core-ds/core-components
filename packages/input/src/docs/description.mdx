## Размеры

Основным типом инпутов в вебе являются инпуты размера M с внутренним расположением лейбла.

```jsx live
render(() => {
    return (
        <div style={{ width: document.body.clientWidth < 450 ? '100%' : 320 }}>
            <Input block={true} label='Текст' size='m' />
            <Gap size='m' />
            <Input block={true} label='Email' size='m' />
        </div>
    );
});
```

## Валидация

См. гайд [валидация](?path=/docs/guidelines-validation--page).

```jsx live
render(() => {
    const [error, setError] = React.useState(false);

    return (
        <div style={{ width: document.body.clientWidth < 450 ? '100%' : 320 }}>
            <div style={{ height: 80 }}>
                <Input block={true} label='Текст' size='m' error={error} />
            </div>
            <Gap size='xl' />
            <Button size='xxs' view='secondary' onClick={() => setError(false)}>
                По умолчанию
            </Button>
            <Gap size='s' direction='horizontal' />
            <Button size='xxs' view='secondary' onClick={() => setError('Error message')}>
                Состояние ошибки
            </Button>
        </div>
    );
});
```

## Disabled и ReadOnly

Доступный только для чтения инпут может быть выбран с клавиатуры, его значение передается при отправке формы.
Заблокированный инпут нельзя выбрать с клавиатуры и его значение не будет отправлено.

```jsx live
render(() => {
    const [inputProps, setInputProps] = React.useState({});

    return (
        <div style={{ width: '100%' }}>
            <div style={{ width: document.body.clientWidth < 450 ? '100%' : 320 }}>
                <Input block={true} label='Текст' size='m' {...inputProps} />
            </div>
            <Gap size='xl' />

            <Space direction='horizontal' wrap={true}>
                <Button size='xxs' view='secondary' onClick={() => setInputProps({})}>
                    По умолчанию
                </Button>
                <Button
                    size='xxs'
                    view='secondary'
                    onClick={() => setInputProps({ disabled: true })}
                >
                    Заблокирован
                </Button>
                <Button
                    size='xxs'
                    view='secondary'
                    onClick={() => setInputProps({ readOnly: true })}
                >
                    Только чтение
                </Button>
            </Space>
        </div>
    );
});
```

## Дополнительная информация

Для упрощения пользовательского ввода используется несколько паттернов вывода дополнительной информации.

1. Placeholder — подсказка внутри поля, отображается до тех пор, пока пользователь не начнет вводить значение.
2. Hint — подсказка под полем. Если поле перейдёт в состоянии ошибки hint будет заменен на сообщение об ошибке.
3. Tooltip — иконка с подсказкой.

```jsx live
render(() => {
    const [open, setOpen] = React.useState(false);

    return (
        <div style={{ width: document.body.clientWidth < 450 ? '100%' : 320 }}>
            <style>{`.target {display: flex;}`}</style>
            <Input
                block={true}
                placeholder='Инпут с плейсхолдером'
                size='m'
                hint='Подсказка под полем'
                rightAddons={
                    <TooltipResponsive
                        open={open}
                        targetClassName='target'
                        content={
                            <span
                                style={{ whiteSpace: 'pre-wrap' }}
                            >{`Сообщение тултипа\nотображается при\nнаведении`}</span>
                        }
                        onOpen={() => setOpen(true)}
                        onClose={() => setOpen(false)}
                        position='right'
                    >
                        <Badge
                            view='icon'
                            iconColor='secondary'
                            content={<InformationCircleMIcon />}
                        />
                    </TooltipResponsive>
                }
            />
        </div>
    );
});
```

## Статусы

В некоторых кейсах может потребоваться дополнительная индикация ошибки или успеха.

```jsx live
render(() => {
    const [success, setSuccess] = React.useState(false);
    const [value, setValue] = React.useState('');

    /* Не показываем вторую иконку ошибки в теме сайта */
    const css = `
        :root {
            --input-error-icon-display: none;
            --input-error-icon-not-alone-display: none;
        }
    `;

    const handleChange = (e) => {
        const value = e.target.value;
        setValue(value);

        if (value.length === 5) {
            setSuccess(true);
        } else {
            setSuccess(false);
        }
    };

    const hasError = value !== '' && !success;

    return (
        <div style={{ width: document.body.clientWidth < 450 ? '100%' : 320 }}>
            <style>{css}</style>
            <Input
                value={value}
                block={true}
                onChange={handleChange}
                placeholder='Введите 5 символов'
                size='m'
                success={success}
                error={hasError}
                rightAddons={
                    hasError ? (
                        <Badge
                            view='icon'
                            size='m'
                            iconColor='negative'
                            content={
                                <ExclamationCircleMIcon
                                    style={{ color: 'var(--color-light-graphic-negative)' }}
                                />
                            }
                        />
                    ) : undefined
                }
            />
        </div>
    );
});
```

## Крестик очистки

В редких кейсах может потребоваться механика быстрой очистки введенного контента.
Рекомендуется использовать данную механику точечно, а не глобально ко всем полям большой формы.

```jsx live
render(() => {
    const [value, setValue] = React.useState('Value');

    return (
        <div style={{ width: document.body.clientWidth < 450 ? '100%' : 320 }}>
            <Input
                value={value}
                block={true}
                onChange={(e) => setValue(e.target.value)}
                label='Label'
                size='m'
                clear={true}
                onClear={() => setValue('')}
            />
        </div>
    );
});
```

## Кликабельные аддоны

Для кликабельных адоннов рекомендуем использовать увеличенную область нажатия.

```jsx live
render(() => {
    const [isVisible, setIsVisible] = React.useState(false);
    const [title, setTitle] = React.useState('');
    const [valueOne, setValueOne] = React.useState('Value');
    const [valueTwo, setValueTwo] = React.useState('Value');

    const toggleVisiblity = () => setIsVisible(prev => !prev) ;
    
    const singleIcon = () => {
        setTitle('Вы восхитительны!')
        toggleVisiblity()
    }
    
    const leftIcon = () => {
        setTitle('Левая')
        toggleVisiblity()
    }
    
    const rightIcon = () => {
        setTitle('Правая')
        toggleVisiblity()
    }
    
    const hideNotification = React.useCallback(() => setIsVisible(false), []);
    
    const styleWrapperIcon = {
        minWidth: '48px',
        height: '100%',
        display: 'flex',
        alignItems: 'center',
        justifyContent: 'center',
        marginRight: 'var(--gap-s-neg)'
    }
    
    const styleWrapperManyIcons = {
        height: '100%',
        display: 'flex',
        marginRight: 'var(--gap-s-neg)'
    }
    
    const styleManyIcons = {
        minWidth: '36px',
        height: '100%',
        display: 'flex',
        alignItems: 'center',
        justifyContent: 'end',
    }

    return (
        <div style={{ width: document.body.clientWidth < 450 ? '100%' : 320 }}>
            <Input
                value={valueOne}
                block={true}
                onChange={(e) => setValueOne(e.target.value)}
                label='Label'
                size='m'
                rightAddons={
                  <div 
                    style={styleWrapperIcon} 
                    onClick={singleIcon}>
                      <DiamondsMIcon color='rgba(116, 116, 116, 1)'/>
                  </div>
                }
            />
            <Gap size='l'/>
            <Input
                value={valueTwo}
                block={true}
                onChange={(e) => setValueTwo(e.target.value)}
                label='Label'
                size='m'
                rightAddons={
                <div style={styleWrapperManyIcons} >
                    <div  style={styleManyIcons} onClick={leftIcon}> 
                        <DiamondsMIcon color='rgba(116, 116, 116, 1)'/>
                    </div>
                    <div style={{...styleManyIcons, justifyContent: 'start'}} onClick={rightIcon}> 
                        <FlowerMIcon color='rgba(116, 116, 116, 1)'/> 
                    </div>
                </div>
              }
            />
            <Notification
                title={title}
                visible={isVisible}
                offset={180}
                onCloseTimeout={hideNotification}
                hasCloser={false}
                autoCloseDelay={2000}
            />
        </div>
    );
})
```

## Морфология

Компонент является оберткой над [FormControl](?path=/docs/formcontrol--form-control), поэтому наследует все доступные для него настройки.
