...
Служат триггерами для переключения контекста, см. компонент [Tabs](?path=/docs/components-tabs--docs).
Используются для фильтрации контента, см. компонент [FilterTag](?path=/docs/components-filtertag--docs).
Выполняют роль радио-кнопок см. компонент [RadioGroup](?path=/docs/components-radiogroup--docs).
Выполняют роль чекбоксов см. компонент [CheckboxGroup](?path=/docs/components-checkboxgroup--docs).
