...
Для finding'а, подтверждённого запуском, приложи воспроизведение, которое автор выполнит у себя: тест или команду, которые падают на последнем коммите PR и проходят после правки. Публикуй воспроизведение в том виде, в каком сам его запускал: незапущенное воспроизведение — это предположение. Если способа воспроизвести нет и проблема выведена только чтением кода (типично для layout: в jsdom `getBoundingClientRect()` и `offsetHeight` нулевые) — скажи это прямо в `evidence` и не ставь severity выше P2.
