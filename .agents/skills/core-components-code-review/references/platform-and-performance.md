...
## Рантайм CSS custom properties (CSS-переменные) как источник значения

Отдельная категория багов — не про сам компонент, а про то, что часть потребителей библиотеки на своей стороне **выпиливает CSS custom properties** (например, PostCSS-плагины вроде `postcss-custom-properties`, инструменты critical-CSS-экстракции или сборки под старые браузеры, статически резолвящие `var(...)` во время билда). Такой инструмент не может вычислить значение custom property, если оно реально приходит не из статического CSS, а из **рантайм-значения**, подставляемого JS-логикой компонента (например, через динамически выбираемый CSS-класс, где разным пропам соответствуют разные переопределения одной и той же переменной) — и в таком случае у потребителя в сборке заинлайнится дефолтное/первое найденное значение вместо фактического. Это визуальный баг, который **не воспроизводится ни в тестах, ни в демке самой библиотеки** (они используют настоящий runtime CSS, без резолва переменных на этапе сборки), поэтому его легко пропустить при обычном ревью.

Реальный пример (`6d31276fe`, `fix(universal-modal): hug content static class`, #2301): `UniversalModal` реализовывал `height="hugContent"` + `margin.top`/`margin.bottom` через custom properties, переопределяемые сгенерированными по значению prop классами:
...
+     max-height: 100%;
      ...
      /* prettier-ignore */
-     @for $i from 0 to 128 {
-         &.topGap-$(i) { --hug-content-top-gap: $(i)px; }
-         &.bottomGap-$(i) { --hug-content-bottom-gap: $(i)px; }
+     @each $top in 0, 2, 4, 8, ... {
+         @each $bottom in 0, 2, 4, 8, ... {
+             &.hugGap-$(top)-$(bottom) {
...
Значение `margin.top`/`margin.bottom` было известно только в рантайме (проп компонента), а до расчёта итогового `max-height` доходило только через `var(--hug-content-top-gap)` в `calc()` — то есть у потребителя, статически резолвящего переменные, `calc()` мог схлопнуться с неверным (дефолтным) числом. Фикс убрал custom property целиком: каждая комбинация `margin.top`/`margin.bottom` теперь — заранее сгенерированный статический класс (`hugGap-{top}-{bottom}`) с готовым числовым `max-height` прямо в CSS, без `var()` в цепочке.

Что проверять в diff по аналогии:
- вводится ли новая custom property, значение которой задаётся **только** через рантайм-класс/inline-`style` в зависимости от prop (а не имеет собственного статического значения в самом правиле, где она используется);
- используется ли такая переменная в `calc()`/другом вычислении, влияющем на layout (`max-height`, `width`, `margin`, `transform` и т.п.) — именно эта комбинация (динамическое значение + вычисление на его основе) ломается при статическом резолве;
- если да — это finding независимо от того, конечно или бесконечно множество возможных значений prop. Такое использование custom property **строго запрещено** в проекте — это не осознанный trade-off, который можно принять. Если значения перечислимы (как в примере выше) — предлагай заменить на статически сгенерированные классы; если формально не перечислимы (например, произвольное число из inline-style) — всё равно указывай на риск и предлагай искать альтернативу (например, инлайновый `style` с самим вычисленным значением вместо custom property + `calc()` в CSS-модуле), а не оставлять как есть.

То же относится к любой JS-логике, которая опирается на строку `var(--foo)`: прокинуть в inline-`style`, распарсить, сравнить. Часть продуктовых команд собирает библиотеку без CSS-переменных (`BUILD_WITHOUT_CSS_VARS`, CI-джоба `search-vars`), и у них такая логика не работает — «это сломается в проектах, которые выпиливают css переменные». Решение переносится в CSS-класс.

## Утечки и изменения в общих пакетах

- **Подписка, таймер, observer или слушатель без снятия в cleanup эффекта** — утечка. В библиотечном компоненте, который у потребителей монтируется и размонтируется постоянно, это P1, а не стилистика. Проверяй каждую пару в diff: `addEventListener`/`removeEventListener`, `setTimeout`/`clearTimeout`, `observe`/`disconnect`, `subscribe`/`unsubscribe`.
- **Таймаут как способ дождаться DOM** — плохой признак («костыль с таймаутом не норм», «лучше избегать таких решений»): обычно есть событие, ref-callback или layout-эффект. Если без него никак — нужен комментарий, почему.
- **Изменение в общем пакете** (`vars`, `shared`, `themes`, `base-modal` и других, от которых зависят десятки пакетов) проверяй по потребителям внутри монорепозитория: grep по использованию изменённой переменной, миксина, утилиты, контекста. Изменение, не учтённое в зависимых пакетах, — P1.
...
| Новая/изменённая CSS custom property, чьё значение задаётся только рантайм-классом/inline-style по prop, участвует в `calc()`/layout-вычислении без статической альтернативы (см. `6d31276fe`)             | **P2** (визуальный баг только у части потребителей, не воспроизводится в тестах/демке библиотеки) |
| Подписка, таймер, observer или слушатель без снятия в cleanup | **P1** |
| Изменение в общем пакете (`vars`, `shared`, `themes` и т.п.), не учтённое в зависимых пакетах | **P1** |
| Таймаут как способ дождаться DOM, без комментария о причине | **P2** |
