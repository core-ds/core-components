...
- **Таймаут как способ дождаться DOM** — плохой признак («костыль с таймаутом не норм», «лучше избегать таких решений»): обычно есть событие, callback-ref или `useLayoutEffect`. Если без него никак — нужен комментарий, почему.
- **Изменение в общем пакете** (`vars`, `shared`, `themes`, `base-modal` и других, от которых зависят десятки пакетов) — случай, когда проверка потребителей из этапа 3 обязательна: найди поиском все использования изменённой переменной, миксина, утилиты или контекста в монорепозитории. Изменение, не учтённое в зависимых пакетах, — P1.
