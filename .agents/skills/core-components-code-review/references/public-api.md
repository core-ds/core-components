...
## Что команда считает мажорным на практике

Список выше — формальный. В истории ревью команды мажорными называли ещё три вида правок, которые по формальному признаку легко пропустить:

- **удаление или переопределение переменных темы и миксинов** — «удаления являются мажорными, так как мы меняем визуальное API»: потребители переопределяют эти переменные у себя;
- **изменение экспортируемого типа или сигнатуры callback'а**, даже если рантайм не меняется, — «это типы, тоже кандидат на major»;
- **появление `default` там, где значения раньше не было вовсе** (например, размера): у потребителей, не передававших prop, рендер изменится так же, как при смене default value.

Как команда с такими правками работает — и что стоит предлагать в `suggested_fix`:

1. Старый prop не удаляют, а помечают `@deprecated` с указанием замены (`@deprecated Используйте scrollLock={true}`); новый добавляется рядом (см. «Мягкий deprecation» ниже).
2. Мажорные правки выносят отдельным PR: «всё что можно минорно — затащим, мажорные правки отдельно сделаем». Если PR смешивает минорную фичу и breaking change — предложи разделить, а не поднимать bump всему PR.
3. **Обратная ошибка — тоже finding:** `major` у изменения, которое публичный контракт не меняет, означает лишний мажорный релиз зонтичного `@alfalab/core-components` для всех потребителей.

### Prop из нативных атрибутов — не публичный API библиотеки

Прежде чем называть сужение или изъятие prop'а breaking change, установи, откуда он взялся. Prop, который компонент не объявляет сам, а наследует из `InputHTMLAttributes`/`HTMLAttributes` и просто прокидывает в DOM-элемент, не влияя на поведение компонента, частью API библиотеки не является.

Реальный пример (#2307, `feat(switch, radio, checkbox)`): `Switch` получил собственный `size?: 20 | 24`, из-за чего из типа ушёл нативный `size?: number` от `InputHTMLAttributes`. Формально — сужение типа при `minor` changeset. Фактически нативный `size` уходил в скрытый `<input type="checkbox">` и ни на что не влиял: это коллизия имён, а не изменение публичного контракта, и `minor` верен. Breaking change — про props, которые компонент объявляет и обрабатывает сам.
...
Из changeset-файлов собирается `CHANGELOG.md`, который читают потребители, поэтому в ревью команды текст changeset правят чаще всего остального. Проверяй:

- **Перечислены только реально изменённые пакеты.** Пакет во frontmatter, в котором diff ничего не меняет, — finding (типичное замечание: «amount-input нет изменений»). Зависимые пакеты получают bump автоматически, вручную их не добавляют.
- **Текст — от лица потребителя и о свершившемся факте:** что изменилось в поведении или API компонента, а не что делал разработчик. Стиль — как в существующих `CHANGELOG.md`: «Исправлен перенос текста опции», «Добавлена иконка замка для readOnly/disabled состояний». Не «фиксим баг», не «рефакторинг», не пересказ реализации.
- **Каждое изменение — отдельный пункт списка, начиная с первого.** Автоформат добавляет дефис только первой строке описания (`docs/contributing.stories.mdx`), остальные строки попадают в `CHANGELOG.md` как есть — два изменения, слитые в одну фразу, там уже не разделить.
- **Идентификаторы — в обратных кавычках:** props, значения, имена компонентов.
- **Deprecation:** сказано, что помечено и что использовать вместо.
- **Изменение поведения по умолчанию названо прямо.** Если после обновления у потребителя без правок кода изменится рендер или геометрия — в том числе у тех, кто обходил исправляемый баг своими средствами, — changeset единственное место, где он об этом узнает.
- **Заголовок `##### ComponentName` в начале описания — не ошибка и в файле на один компонент:** `bin/extend-changeset.mjs` при `version-packages` срезает ведущий заголовок и генерирует его заново из списка пакетов. Не проси ни убрать его, ни добавить.
...
| Нарушено оформление: нет `#####`-заголовков при нескольких логически связанных компонентах в одном файле (заголовок в файле на один компонент — не нарушение, см. «Текст changeset») | **P3** |
| Changeset создан для пакета из `ignore`-списка `.changeset/config.json` | **P3** (избыточно, не блокирует) |
| `major` changeset у изменения, которое публичный контракт не меняет | **P2** (лишний мажорный релиз зонтичного пакета) |
| Сужение/изъятие prop'а, унаследованного из нативных атрибутов и не влиявшего на поведение компонента | не finding |
| Во frontmatter changeset указан пакет, в котором diff ничего не меняет | **P2** |
| Изменение поведения по умолчанию не названо в тексте changeset | **P2** |
| Текст changeset пересказывает реализацию, несколько изменений слиты в один пункт, идентификаторы без обратных кавычек | **P3** |
