...
if (process.env.BUILD_STORYBOOK_FROM_DIST === 'true') {
    try {
        componentsContext = require.context(
            '../dist',
            true,
            /^\.\/(.*)\/modern\/(index|desktop|mobile|responsive|circle|super-ellipse|rectangle|no-shape).js$/,
        );
    } catch (e) {
    ...
    componentsContext = require.context(
        '../packages',
        true,
        /^\.\/(.*)\/src\/(index|desktop|mobile|responsive|circle|super-ellipse|rectangle|no-shape).ts$/,
    );
}
